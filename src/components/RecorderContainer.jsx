...
const RecorderContainer = () => {
    ...
    const generateReport = () => {
        ...
        const content = `
=========================================
      RAKSHAVAANI SECURITY REPORT
=========================================
Date: ${date}
Scan Status: ${result.riskLevel.toUpperCase()}
Confidence Score: ${result.score}%

-----------------------------------------
ANALYSIS DETAILS
-----------------------------------------
Classification: ${result.riskLevel === 'Fraud' ? 'HIGH RISK' : result.riskLevel === 'Suspicious' ? 'MEDIUM RISK' : 'SAFE'}
Primary Reason: ${result.reasons.join(', ')}
...
${result.riskLevel === 'Fraud'
                ? "IMMEDIATE ACTION: Do not share any OTP/PIN. Block this number. Report to 1930."
                : "SAFE: This call appears to be legitimate. Always stay vigilant."}

=========================================
Generated by RakshaVaani AI Neural Engine
        `;
        ...
        a.href = url;
        a.download = `RakshaVaani_Report_${Date.now()}.txt`;
    ...
    useEffect(() => {
        // Use either mic blob or uploaded blob
        const blobToProcess = audioBlob || uploadedBlob;

        if (status === 'processing' && !isRecording && blobToProcess) {
            const runAnalysis = async () => {
                ...
                try {
                    ...
                    reader.onloadend = async () => {
                        ...
                        try {
                            // 2. Call the new Voice Detection API
                            console.log("Sending to API...");
                            const response = await fetch('http://localhost:3000/api/voice-detection', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'x-api-key': import.meta.env.VITE_GEMINI_API_KEY || 'sk_test_123456789'
                                },
                                body: JSON.stringify({
                                    language: "English", // Defaulting to English for demo
                            ...
                            // 3. Map API Response to UI State
                            const analysisResult = {
                                riskLevel: data.classification === 'AI_GENERATED' ? 'Fraud' : 'Safe',
                                score: Math.round(data.confidenceScore * 100),
                                transcript: data.explanation, // Using explanation as the text display
                                riskyPhrases: [], // API doesn't return these yet, keeping empty
                                reasons: [data.classification]
                            };
                            ...
                            // 4. Force state transition after small delay
                            setTimeout(() => {
                            ...
                            // Fallback to offline mock if server is dead (Graceful degradation)
                            console.log("Switching to offline fallback...");

                            // Ensure analyzeAudio exists before calling
                            try {
                                const offlineResult = await analyzeAudio(blobToProcess);
                                clearInterval(interval);
                                setProgress(100);
    ...
    }, [status, isRecording, audioBlob, uploadedBlob]); // Added uploadedBlob dep
                            ...
                            <div className="flex flex-col lg:flex-row gap-16 items-start">
                                ...
                                <div className="flex-1 space-y-12">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-6 pt-6">
                                        ...
                                        <button onClick={generateReport} className="btn-secondary-outline flex-1 py-5 text-lg">Generate AI Report</button>
